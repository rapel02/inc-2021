...
def main(argv):
  ...
  N = vr.ReadInt(2, 500)
  vr.ReadSpace()
  M = vr.ReadInt(1, N * (N - 1))
  vr.ReadSpace()
  T = vr.ReadInt(1, N)
  vr.ReadEndl()

  L = vr.ReadIntList(T, 1, N)

  assert len(set(L)) == T, "treasure must in distinct places"

  edges_set = set({})

  for i in range(M):
    u, v = vr.ReadIntList(2, 1, N)
    assert u != v, "graph cannot contains self loop"
    edges_set.add((u, v))

  assert len(edges_set) == M, "edges cannot contains parallel"
